...
def test_metadata_state_migrates_existing_source_records(tmp_path) -> None:
    ...
    with sqlite3.connect(state_db) as conn:
        ...
        conn.execute(
            ...
            (
                ...
                "en",
                "text",
                json.dumps({"id": "1", "url_l": "https://live.staticflickr.com/1.jpg"}),
    ...
    assert row["all_query_labels"] == ["text:Papilio"]
...
def test_poll_once_fetches_metadata_only_dedupes_and_queues_image_urls(tmp_path) -> None:
    ...
    frame = pl.read_parquet(tmp_path / "evidence" / "poll.parquet")
...
def test_poll_once_exports_canonical_evidence_without_duplicate_existing_rows(tmp_path) -> None:
    state = MetadataPollState(tmp_path / "poller.sqlite")
...
def test_poll_once_folds_duplicate_query_terms_onto_source_record(tmp_path) -> None:
    ...
    with sqlite3.connect(state.path) as conn:
        ...
        query_hit_count = conn.execute("SELECT count(*) FROM source_record_query_hits").fetchone()[0]
    assert source_rows[0][0:2] == ("text", "Papilio")
    assert json.loads(source_rows[0][2]) == ["Papilio"]
    assert json.loads(source_rows[0][3]) == ["Papilionidae"]
    assert json.loads(source_rows[0][4]) == ["text:Papilio", "tags:Papilionidae"]
    assert source_rows[0][5:] == (2, 0)
    assert query_hit_count == 0
...
def test_poll_once_keeps_one_source_record_and_tracks_image_url_history(tmp_path) -> None:
    ...
    with sqlite3.connect(state.path) as conn:
        source_rows = conn.execute(
            """
            SELECT flickr_photo_id, image_url, text_search_terms_json, tag_search_terms_json,
                   all_query_labels_json, query_hit_count, duplicate_query_hit_count
            FROM source_records
            """
        ).fetchall()
    ...
    assert source_rows[0][0:2] == ("1", "https://live.staticflickr.com/1_large.jpg")
    assert json.loads(source_rows[0][2]) == ["Papilio"]
    assert json.loads(source_rows[0][3]) == ["Papilio"]
    assert json.loads(source_rows[0][4]) == ["text:Papilio", "tags:Papilio"]
    assert source_rows[0][5:] == (2, 0)
    assert url_rows == [
...
def test_poll_once_does_not_append_duplicate_query_terms_on_rerun(tmp_path) -> None:
    state = MetadataPollState(tmp_path / "poller.sqlite")
    state.enqueue_work_item(FlickrQuery(term="Papilio", language="en", search_field="text", lane="normal_page", page=1, per_page=250))
    ...
    with sqlite3.connect(state.path) as conn:
        row = conn.execute(
            """
            SELECT text_search_terms_json, all_query_labels_json,
                   query_hit_count, duplicate_query_hit_count
            FROM source_records
            """
        ).fetchone()
