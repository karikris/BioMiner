...
def test_media_resources_do_not_equate_urls_with_content(tmp_path: Path) -> None:
    ...
    pq.write_table(pa.Table.from_pylist([
        _row("m1","1","h","https://x/a.jpg","CONFLICT"),
...
def _row(media_id,gbif_id,url_hash,url,cross_taxon):
    return {"media_assertion_id":media_id,"gbifID":gbif_id,"canonical_url_hash":url_hash,"canonical_url":url,"canonical_url_group_size":2 if url_hash else None,"source_platform_group_id":"platform","url_distinct_occurrences":2 if url_hash else None,"url_distinct_taxa":2 if cross_taxon=="CONFLICT" else 1,"url_distinct_licenses":1,"cross_taxon_url_status":cross_taxon,"cross_license_url_status":"PASS"}
