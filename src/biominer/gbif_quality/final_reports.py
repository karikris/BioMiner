...
def publish_final_reports(*, data_root: str | Path, report_root: str | Path, code_commit: str) -> dict[str, object]:
    ...
    try:
        contents={
            ...
            "bias_and_representativeness.md": _report("Bias and representativeness", "Counts distinguish raw assertions, occurrences, exact URLs, canonical URLs, and URL-adjusted support. Content/perceptual adjustment is NOT_TESTED; absence in media data does not imply biological absence.", _mapping(representation["counts"]), _table(c,data/"representativeness/coverage_by_dimension.parquet", "SELECT dimension,\"value\",raw_image_count,distinct_occurrence_count,duplicate_adjusted_count,unresolved_count FROM read_parquet(?) WHERE dimension IN ('provider','dataset') ORDER BY raw_image_count DESC LIMIT 40"), _table(c,data/"representativeness/species_bias_flags.parquet", "SELECT species,raw_image_count,distinct_occurrence_count,duplicate_adjusted_count,provider_count,creator_count,country_count,decade_count,bias_flags FROM read_parquet(?) ORDER BY duplicate_adjusted_count DESC LIMIT 30")),
