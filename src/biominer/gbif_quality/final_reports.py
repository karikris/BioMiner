...
def publish_final_reports(*, data_root: str | Path, report_root: str | Path, code_commit: str) -> dict[str, object]:
    ...
    required = [
        ...
        data / "quality_results/restart_validation/manifest.json",
    ...
    recovery=load(data/"quality_results/restart_validation/manifest.json")
    ...
    try:
        contents={
            ...
            "performance_and_reproducibility.md": _report("Performance and reproducibility", f"Six full-data benchmark stages passed under the configured 8 GiB analytical-engine limit. Observed process high-water RSS was {performance['counts']['peak_rss_bytes']:,} bytes, below the 16 GiB acceptance ceiling. Incremental semantic equality and zero changed rows prove deterministic value-level rerun behavior. Restart validation checksum-verifies and skips {recovery['counts']['skipped_committed_stages']} committed stages and found {recovery['counts']['orphaned_staging_directories']} orphaned staging directories. The manual-review population contains {reviews['counts']['capsule_rows']:,} deterministic sealed capsules. No live resolver or provider enrichment is claimed.", _table(c,data/"performance/benchmark_results.parquet", "SELECT stage,status,rows_read,elapsed_seconds,rows_per_second,input_bytes,process_peak_rss_bytes,result_fingerprint FROM read_parquet(?) ORDER BY stage"), _mapping(incremental["counts"]), _mapping(incremental["validation"]), _mapping(recovery["validation"]), _mapping(freshness["counts"]), _mapping(gates["counts"])),
        }
    finally: c.close()
    generated=[]
