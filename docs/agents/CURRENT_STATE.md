...
- as of 2026-07-19T00:00:00+10:00 local (workspace check), no local live
  Biominer process is active; recent `run`/`logs`/`data/state` artifacts are
  complete or historical. The GBIF 18 July 2026 download has been materialized
  to `data/reference/gbif_occurrences.parquet` (571,755 rows) with manifest
  `data/reference/gbif_occurrence_manifest.json` from DOI
