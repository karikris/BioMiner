...
| `src/biominer/detection/policy.py` | Defines YOLO, crop and Mac runtime settings plus a fingerprinted eight-route policy. Possible-adult scoring and ambiguous-insect review have independent enable flags and inclusive thresholds. | Keep profile settings identical across local/cloud paths and bind the route-policy fingerprint into every target-aware work identity. |
| `src/biominer/detection/pipeline.py` | Runs bounded detection and emits `object-detection-v2` rows with raw prompt/class provenance, route-aware NMS, explicit no-organism/failure states, route decisions, and crop metadata for current score/review routes. | Keep YOLO as gate/router and retain all detection metadata. Target-aware processing must stop materializing detector crops; subject-size/detail signals become abstention features. |
| `src/biominer/vision/gates.py` | Uses a fail-closed routed gate, validates adult/larval/specimen route pairs and policy identity, never scores review rows, and forbids no-detection fallback. | Every target-aware scored route must use complete-canvas inputs and a route-compatible scorer; add compatible larval/specimen scorers without weakening adult isolation. |
| `src/biominer/run/orchestrator.py` | Requires a pinned reference-bank readiness permit before vision and runs the current adult classifier behind an adult-only routed gate in local and cloud paths. | Add the target-aware stage identities and full-frame scorer while retaining the readiness and route-compatibility checks. |
| `src/biominer/run/stages.py` | Originally defined the linear legacy discovery workflow; the comment-promotion stages were removed on 2026-07-19. | Add geographic, clustering, candidate, reference, embedding, training, calibration, readiness and target-scoring stages. Manual review cannot auto-complete. |
...
| `src/biominer/evaluation/labels.py` | Requires the complete reviewed-label v2 contract, including target presence, life stage/domain, duplicate/owner groups, dataset split and second review. | Producers must write all current fields explicitly; historical v1 labels are not accepted by the production reader. |
