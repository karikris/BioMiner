...
| `src/biominer/vision/gates.py` | Defaults to a fail-closed routed gate, validates adult/larval/specimen route pairs and policy identity, never scores review rows, and forbids no-detection fallback. Legacy collapsed-label gates are explicit diagnostics. | Every target-aware scored route must use complete-canvas inputs and a route-compatible scorer; add compatible larval/specimen scorers without weakening adult isolation. |
