...
| `src/biominer/registry/publish.py` | Publishes only the seven core unified-registry artifacts after fatal QA; current country range artifacts are not in the publication contract. | Extend the required publication set atomically with geographic artifacts and their checksums after geographic QA passes. |
